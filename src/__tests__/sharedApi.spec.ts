...
describe('encodeArguments – JSON strategy', () => {
	...
	test('Uint8Array is base64-encoded via its underlying ArrayBuffer', () => {
		const u = new Uint8Array([0x00, 0x80, 0xff])
		const [enc] = encodeArguments({}, {}, [u], S)
		expect(enc.type).toBe('buffer')
		expect(typeof enc.value).toBe('string')
		// should equal a Buffer wrapping the same bytes
		expect(enc.value).toBe(
			Buffer.from(u.buffer, u.byteOffset, u.byteLength).toString(
				'base64'
			)
		)
	})

	test('Uint8Array with full binary range round-trips correctly (decoded as Buffer)', () => {
		const u = new Uint8Array(BINARY_BYTES)
		const [decoded] = roundTrip([u], S)
		expect(Buffer.isBuffer(decoded)).toBe(true)
		// bytes must be preserved exactly
		expect(decoded.equals(Buffer.from(u))).toBe(true)
	})

	test('Uint8Array subarray (non-zero byteOffset) round-trips correctly', () => {
		// A sub-array shares a parent ArrayBuffer with a non-zero byteOffset.
		// Encoding must use byteOffset/byteLength, not the whole buffer.
		const parent = new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04])
		const sub = parent.subarray(1, 4) // [0x01, 0x02, 0x03]
		const [decoded] = roundTrip([sub], S)
		expect(Buffer.isBuffer(decoded)).toBe(true)
...
describe('encodeArguments – StructuredClone strategy', () => {
	...
	test('Uint8Array is stored as-is (not base64)', () => {
		const u = new Uint8Array([0x00, 0x80, 0xff])
		const [enc] = encodeArguments({}, {}, [u], S)
		expect(enc.type).toBe('buffer')
		expect(typeof enc.value).not.toBe('string')
	...
	test('Uint8Array round-trips correctly, decoded as Buffer sharing the ArrayBuffer', () => {
		const u = new Uint8Array(BINARY_BYTES)
		const [enc] = encodeArguments({}, {}, [u], S)
		// simulate structured clone delivery: Uint8Array backed by a fresh ArrayBuffer
		const arrived = new Uint8Array(
			enc.value.buffer,
			enc.value.byteOffset,
			enc.value.byteLength
		)
		const syntheticEnc: ArgDefinition = { ...enc, value: arrived }
		const [decoded] = decodeArguments(() => ({}), [syntheticEnc], (() => {
			throw new Error()
		}) as any)
		expect(Buffer.isBuffer(decoded)).toBe(true)
		expect(decoded.equals(Buffer.from(u))).toBe(true)
		expect(decoded.buffer).toBe(arrived.buffer)
	})
})
...
describe('encodeArguments – InProcess strategy', () => {
	...
	test('Uint8Array original reference is preserved', () => {
		const u = new Uint8Array([1, 2, 3])
		const [enc] = encodeArguments({}, {}, [u], S)
		expect(enc.type).toBe('buffer')
		expect(enc.original).toBe(u)
		expect(enc.value).toBeNull()
		const [decoded] = decodeArguments(() => ({}), [enc], (() => {
			throw new Error()
		}) as any)
		expect(decoded).toBe(u) // still same reference
	})
