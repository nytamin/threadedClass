...
type CtorArgs<CtorT extends new (...args: any) => any> = CtorT extends new (
	...args: infer K
) => any
	? K
	: never
...
export function threadedClass<T, TCtor extends new (...args: any) => T> (
	...
	if ((typeof orgModule as any) !== 'string') {
		throw new Error('threadedClass parameter orgModule must be a string!')
	}
	if ((typeof orgExport as any) !== 'string') {
	...
	return new Promise<ThreadedClass<T>>((resolve, reject) => {
		function sendFcn (
			...
			ThreadedClassManagerInternal.sendMessageToInstance(
				instance,
				msg,
				cb
			)
		...
		function replyError (
			instance: ChildInstance,
			msg: Message.From.Instance.Callback,
			error: Error
		) {
		...
		function decodeResultFromWorker (
			instance: ChildInstance,
			encodedResult: any
		) {
							...
							sendCallback(
								...
								(_instance, err, encodedResult) => {
									...
									if (err) {
										...
										let result = decodeResultFromWorker(
											_instance,
											encodedResult
										)
										resolve(result)
		...
		function onMessageFromInstance (
			instance: ChildInstance,
			m: Message.From.Instance.Any
		) {
			if (m.cmd === Message.From.Instance.CommandType.REPLY) {
				let msg: Message.From.Instance.Reply = m
				const child = instance.child
				let cb: InstanceCallbackFunction =
					child.instanceMessageQueue[msg.replyTo + '']
				if (!cb) return
				if (msg.error) {
				...
				if (callback) {
					try {
						Promise.resolve(callback(...msg.args))
							.then((result: any) => {
								let encodedResult = encodeArguments(
									instance,
									instance.child.callbacks,
									[result],
									instance.child.process.encodingStrategy
								)
								sendReplyToInstance(
									instance,
									msg.cmdId,
							...
							.catch((err: Error) => {
								try {
									...
									ThreadedClassManagerInternal.debugLogError(
										new Error(
											`ThreadedClass: Thrown error in callback "${msg.callbackId}": Unable to forward error to child due to: ${e}`
										)
									)
								}
							})
					...
					throw Error(
						`callback "${msg.callbackId}" not found in instance ${m.instanceId}`
					)
				}
			}
		}
		try {
			...
			if (isBrowser()) {
				...
				pathToWorker = thisCallPath
					...
					.replace(
						/threadedClass(\.[tj]s)$/,
						'threadedclass-worker.js'
					)
					.replace(
						/src([\\\/])child-process([\\\/])threadedclass-worker/,
						'dist$1child-process$2threadedclass-worker'
					)
			}

			const child: Child =
				ThreadedClassManagerInternal.findNextAvailableChild(
					config,
					pathToWorker
				)
			...
			let instanceInChild: ChildInstance =
				ThreadedClassManagerInternal.attachInstanceToChild(
					config,
					child,
					proxy,
					pathToModule,
			...
			ThreadedClassManagerInternal.sendInit(
				child,
				instanceInChild,
				config,
				(
					...
					if (err) {
						...
						props.forEach((p: InitProp) => {
							if (!instance.child.alive) {
								throw Error(
									`Child process of instance ${instance.id} has been closed`
								)
							}
								...
								const callMethod = (...args: any[]) => {
									...
									if (!instance.child) {
										return Promise.reject(
											new Error(
												`Instance ${instance.id} has been detached from child process`
											)
										)
									}

									return ThreadedClassManagerInternal.doMethod(
										instance.child,
										p.key,
										(resolve, reject) => {
											if (!instance.child) {
											...
											let encodedArgs = encodeArguments(
												instance,
												instance.child.callbacks,
												args,
						...
						ThreadedClassManagerInternal.startMonitoringChild(
							instanceInChild
						)
						resolve(proxy)
						ThreadedClassManagerInternal.checkInstance(
							instanceInChild,
							errStack
						)
						return true
