		...
		return args.map((arg, i): ArgDefinition => {
			try {
				...
				if (strategy === EncodingStrategy.StructuredClone) {
					...
					// Pass the Buffer as-is; it arrives on the other side as a Uint8Array.
					if (arg instanceof Buffer) {
						return { type: ArgumentType.BUFFER, value: arg }
					}
				}

				// EncodingStrategy.JSON: Buffer must be base64-encoded because JSON.stringify loses binary data
				if (arg instanceof Buffer) {
					return {
						type: ArgumentType.BUFFER,
...
export function decodeArguments (
	...
	return args.map((a) => {
		...
		if (a.type === ArgumentType.BUFFER) {
			...
			// Buffer.from(Uint8Array) wraps the same ArrayBuffer – zero extra copy.
			if (typeof a.value === 'string') {
				return Buffer.from(a.value, 'base64')
			}
			return Buffer.from(a.value as Uint8Array)
		}
